      ...
      <Layout style={{ minHeight: '100vh' }}>
        ...
        <Layout>
          <Layout.Header style={{ background: '#fff', minHeight: 52, padding: 0 }}>
            <Header collapsed={this.state.collapsed} toggle={this.toggle}  onSelectChange={() => {}}/>
          </Layout.Header>
              ...
              {appRouters
                ...
                .map((route: any, index: any) => {
                  if (route.children && route.children.length > 0) {
                    return route.children.map((childRoute: any, childIndex: any) => (
                      <Route
                        exact
                        key={childIndex}
                        path={childRoute.path}
                        render={(props) => (
                          <ProtectedRoute component={childRoute.component} permission={childRoute.permission} />
                        )}
                      />
                    ));
                  }
