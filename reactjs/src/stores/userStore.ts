...
import { GetRoles, GetOrganization } from '../services/user/dto/getRolesOuput';
...
class UserStore {
  ...
  async delete(entityDto: EntityDto) {
    ...
    this.users.items = this.users.items
        .filter((x: GetUserOutput) => x.id !== entityDto.id)
  }
  ...
  async getOrganization() {
    let result = await userService.getOrganization();
