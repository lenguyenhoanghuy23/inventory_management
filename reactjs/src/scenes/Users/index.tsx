...
class User extends AppComponentBase<IUserProps, IUserState> {
  ...
  async createOrUpdateModalOpen(entityDto: EntityDto) {
    if (entityDto.id === 0) {
      ...
      await this.props.userStore.getOrganization();
    } else {
  ...
  handleCreate = () => {
    ...
    const {organization} = this.props.userStore
    form!.validateFields().then(async (values: any) => {

      const orgNames = values.orgNames.map((displayName: string) => {
        const org = organization.find(org => org.displayName === displayName);
        return org ? org.id : null;
      });
      if (this.state.userId === 0) {
        const updatedValues = { ...values, orgNames };
        await this.props.userStore.create(updatedValues);
      } else {
        
        const updatedValues = { ...values, orgNames };
        console.log(updatedValues);
        await this.props.userStore.update({ ...updatedValues, id: this.state.userId });
      }
  ...
  handleSearch =(e: React.ChangeEvent<HTMLInputElement>)  => {
    ...
    this.setState({ filter: searchValue }, async () => await this.getAll());
  };
  ...
  public render() {
    ...
    const columns = [
      ...
      { title: L('orgNames'), dataIndex: 'orgNames', key: 'orgNames', width: 150, render: (text: string[] ) =><div>{text.join(',')}</div> },
      ...
      <Card>
        ...
        <Row>
          <Col sm={{ span: 10, offset: 0 }}>
            <Input placeholder={this.L('Filter')} onChange={(e: React.ChangeEvent<HTMLInputElement>) => this.handleSearch(e)} />
          </Col>
        </Row>
        ...
        <CreateOrUpdateUser
          ...
          org={this.props.userStore.organization}
        />
      </Card>
    );
  }
