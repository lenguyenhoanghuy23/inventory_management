...
using emes.Authorization.Accounts;
...
using static emes.Authorization.Organization.Organization;
...
namespace emes.Users
{
    ...
    public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UpdateUserDto>, IUserAppService
    {
        ...
        IRepository<UserOrganizationUnit, long> _userOrgRepository;
        
        readonly private IRepository<OrganizationUnit, long> _OrgRepository;
        ...
        public override async Task<UserDto> CreateAsync(CreateUserDto input)
        {
            ...
            await _userManager.SetOrganizationUnitsAsync(user, input?.orgNames);
        ...
        {
            var OrgUser = _userManager.GetOrganizationUnitsAsync(user).Result;
            //var UserInOrg = _userManager.GetUsersInOrganizationUnitAsync(OrgUser.FirstOrDefault(), false);
           
            var roleIds = user.Roles.Select(x => x.RoleId).ToArray();
            var roles = _roleManager.Roles.Where(r => roleIds.Contains(r.Id)).Select(r => r.NormalizedName);
            
            var OrgUnit = OrgUser.Select(x => x.DisplayName).ToArray();
            
            var userDto = base.MapToEntityDto(user);
            userDto.RoleNames = roles?.ToArray();
            userDto.OrgNames = OrgUnit?.ToArray();
            return userDto;
        }
